...
//api key
const API_KEY = "at_dhWXSV9RgDFGGdIjpLN9mikkBCW6O";
...
class InputForm extends Component {
  //click handling
  clickHandler() {
    //fetching the api for locaton

    fetch(
      `https://geo.ipify.org/api/v1?apiKey=${API_KEY}&ipAddress=${this.state.search}`
    )
      .then((res) => res.json())
      .then((data) => {
        this.setState({
  ...
  constructor(props) {
    super(props);
    //declaring all the different states
    this.state = {
      search: "",
      location: {},
      lat: "",
